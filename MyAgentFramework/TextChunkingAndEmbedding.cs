...
namespace MyAgentFramework
{
    public class TextChunkingAndEmbedding//(ITestOutputHelper output) : BaseTest(output)
    {
        ...
        //private const string EmbeddingModelName = "qwen3-embedding";
        private static readonly Tokenizer s_tokenizer = TiktokenTokenizer.CreateForModel(EmbeddingModelName);

        //[Fact]
        public async Task RunAsync()
        {
            Console.WriteLine("======== [START] Text Embedding ========");
            await RunExampleAsync();
            Console.WriteLine("======== [END] Text Embedding ========");
        }

        private async Task RunExampleAsync()
        {
            var modelId = "qwen3-embedding";
            var embeddingGenerator = new OpenAIClient(new ApiKeyCredential("apikey"), new OpenAIClientOptions()
            {
...
#pragma warning disable SKEXP0050
            // 切分段落時，預留 10~20% 的緩衝空間
            var lines = TextChunker.SplitPlainTextLines(ChatTranscript, maxTokensPerLine: 100);
            var paragraphs = TextChunker.SplitPlainTextParagraphs(lines, maxTokensPerParagraph: 256);
